...
```{r moh_wide_screen}
moh_ws<-xlsx::read.xlsx2('./data/founder_variants_lists/moh/wide_screen_2024.v2.xlsx',
                         sheetIndex = 1)
moh_ws$X.<-NULL
...
validated_moh_ws_vep_annotated<-validated_moh_ws_vep_annotated%>%
  mutate(valid_chr=ifelse(grepl('^[1-9XY]',chr),1,0))%>%
  group_by(variant_id)%>%
  slice_max(valid_chr)%>%ungroup()
# remove same pos variants
# validated_moh_ws_vep_annotated<-validated_moh_ws_vep_annotated%>%
#   distinct(hgvs_notation,chr,start,end,ref,alt,.keep_all = TRUE)

# compare with full founders list
full_founders_list<-readr::read_delim('./output/combined_founder_variants_table.2024-04-25.csv')
full_founders_list<-full_founders_list%>%
...
moh_variants_in_full_list<-
  validated_moh_ws_vep_annotated%>%
  filter(
    (transcript_hgvs%in%full_founders_list$transcript_hgvs)|(gene_hgvs%in%full_founders_list$gene_hgvs)
  )%>%pull(variant_id)

variants_in_moh_not_in_full_founders<-validated_moh_ws_vep_annotated%>%
  filter(!(variant_id%in%moh_variants_in_full_list))

variants_in_moh_not_in_full_founders<-validated_moh_ws_vep_annotated%>%
  #select(variant_id,moh_hgvs_notation=hgvs_notation,chr,start,end,ref=genomic_ref,alt=genomic_alt)%>%distinct()%>%
  select(variant_id,moh_hgvs_notation=hgvs_notation,hgvsc,gene,chr,start,end,ref=genomic_ref,alt=genomic_alt)%>%distinct()%>%
  left_join(
    full_founders_list%>%select(hgvsc,chr,start,end,ref,alt,source,gene),
    by=c('hgvsc')
    )

found_variant_ids<-variants_in_moh_not_in_full_founders%>%filter(!is.na(gene.y))%>%pull(variant_id)%>%unique()
variants_in_moh_not_in_full_founders<-variants_in_moh_not_in_full_founders%>%filter(is.na(gene.y),!(variant_id%in%found_variant_ids))

full_founders_list%>%filter(grepl('NM_000642',hgvsc))
...
moh_ambiguous_table<-moh_ws%>%
  select(variant_id,original_hgvs_notation)%>%
  left_join(validated_moh_ws_vep_annotated,by='variant_id')%>%
  filter(variant_id%in%ambiguous_variant_ids)

xlsx::write.xlsx2(moh_ambiguous_table%>%
                    select(original_hgvs_notation,
                           ...
                           hgvsp)%>%
                    as.data.frame(),
                  file=glue('./output/moh_ambiguous_variants.{Sys.Date()}.xlsx'),
