...
founders_for_merge<-readr::read_delim('./output/combined_founder_variants_table.2024-07-31.csv')%>%
...
# make sure the MOH ws is not in the founders for merege 
founders_for_merge<-founders_for_merge%>%filter(source!='moh_unified_screen')
...
final_founder_table<-
  final_founder_table%>%mutate(old_start=start,old_end=end,
...
final_founder_table_no_dups<-
  final_founder_table%>%
  group_by(chr,start,end,genomic_ref,genomic_alt,assembly_name,gene_symbol,hgvsc,hgvsp)%>%
...
# write bed file
bed_founders_no_dups<-final_founder_table_no_dups%>%
  ungroup()%>%
  select(chr,start,end,gene_symbol,hgvsc)

write.table(bed_founders_no_dups,file=glue('./output/combined_founder_variants_table.no_duplicates.{Sys.Date()}.bed'),row.names = F,col.names = F,quote = F,sep='\t')
```
